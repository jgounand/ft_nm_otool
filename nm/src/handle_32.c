...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/17 14:02:37 by jgounand          #+#    #+#             */
/*   Updated: 2019/09/17 14:08:07 by jgounand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/ft_nm.h"

static char		*ft_get_name_32(char type, struct nlist list,
		t_inf_header *info, void *stringtable)
{
	...
	if (stringtable + list.n_un.n_strx < info->file || stringtable +
			list.n_un.n_strx > info->file + info->size)
		return ("bad string index");
	else
		return (stringtable + list.n_un.n_strx);
}

static t_list	*parse_symtab_32(struct nlist *array, t_inf_header *info,
		struct symtab_command *sym)
{
	...
	while (i < sym->nsyms)
	{
		if (array[i].n_type & N_STAB && ++i)
			continue;
		if ((new.sym_type = ft_get_type_32(array[i], info)) == 1)
			return (NULL);
		new.sym_name = ft_get_name_32(new.sym_type, array[i], info,
				stringtable);

...
static int	position_header(struct fat_header  *header, cpu_type_t cpusearch, bool swap, t_inf_header *info)
{
	uint32_t		i;
	int		found;
...
int	handle_fat(t_inf_header *info)
{
	...
	if (addr_outof_range(info, arch +  header->nfat_arch))
		return (EXIT_FAILURE);
	if ((index = position_header(header, CPU_TYPE_X86_64,info->swap,info)) >= 0)
	{
		arch += index;
